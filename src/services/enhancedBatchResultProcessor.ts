...
export async function processEnhancedBatchResults(
  ...
  console.log(`[ENHANCED PROCESSOR] === PROCESSING ${processedResults.length} RESULTS WITH CHUNKED KEYWORD EXCLUSION AND SIC VALIDATION ===`);
  ...
  const { results: chunkResults } = await processInChunks(
    processedResults,
    async (processedResult, i) => {
    ...
    {
      chunkSize: processedResults.length > 5000 ? 100 : 50,
      delayMs: processedResults.length > 1000 ? 15 : 10,
      onProgress: onProgress
    }
  );

  // Add all processed results
  finalClassifications.push(...chunkResults);
  ...
  console.log(`[ENHANCED PROCESSOR] === PROCESSING COMPLETE WITH CHUNKED KEYWORD EXCLUSION AND SIC VALIDATION ===`);
...
export function processEnhancedBatchResultsSync(
  processedResults: TrueBatchClassificationResult[],
  uniquePayeeNames: string[],
  payeeData: PayeeRowData,
  job: BatchJob
